...
private:
    ...
    static bool ctrlKeys[256];                              // vetor de controle do pressionamento de teclas
...
// retorna verdadeiro na primeira vez que a tecla é pressionada, retorna falso enquanto a tecla não for solta
inline bool Window::KeyClicked(int vkcode)
{
    if (KeyDown(vkcode) && !ctrlKeys[vkcode]) {
        ctrlKeys[vkcode] = true;
        return true;
    }
    else if (KeyUp(vkcode)) {
        ctrlKeys[vkcode] = false;
    }
    return false;
}
